    ...
    try:
        ...
        if not DEBUG:
            with console.status("[dim]Running...[/dim]", spinner="dots"):
                result = subprocess.run(cmd, shell=True, text=True, capture_output=True, timeout=SUBPROCESS_TIMEOUT, env=safe_env)
        else:
            result = subprocess.run(cmd, shell=True, text=True, capture_output=True, timeout=SUBPROCESS_TIMEOUT, env=safe_env)
...
# --- LLM & AGENT ---
llm = ChatOllama(model=MODEL, temperature=0.1, num_ctx=CTX_LENGTH)
...
if __name__ == "__main__":
    ...
    if not sys.stdin.isatty():
        ...
        console.print(Markdown(llm.invoke([SystemMessage(content="Analyze this."), HumanMessage(content=prompt)]).content))
        sys.exit(0)

    # 2. Single Command
    if args.prompt:
        try:
            res = agent.invoke({"messages": [HumanMessage(content=" ".join(args.prompt))]}, config={"callbacks": [SpinnerHandler(console)]})
            console.print(Markdown(res['messages'][-1].content))
        except (UserAbort, KeyboardInterrupt): console.print("[yellow]Aborted.[/yellow]")
        sys.exit(0)
    ...
    while True:
        ...
        except UserAbort: console.print("[yellow]Aborted.[/yellow]")
